...
class AuctionClient:
    def __init__(self, value, clients_num, insurances_num):
        ...
        self.current_bid = self.value * 0.9
        self.increment = self.value * 0.02
    def decide_bid(self, t, duration):
        # If the duration is less than 0.9 years, return -1 as the bid since it's not worth it
        if duration < 1:
            return -1

        # Adjust the bid based on the duration of the insurance
        adjusted_bid = self.current_bid * (duration / 3)

        # Ensure bid gives positive utility
        expected_utility = duration * (self.value - adjusted_bid)
    ...
    def update(self, t, price):
        if price <= self.current_bid:
            min_bid = self.value * 0.5
            self.current_bid = max(min_bid, self.current_bid - self.increment)
        else:
            self.current_bid = min(self.value, self.current_bid + self.increment)
